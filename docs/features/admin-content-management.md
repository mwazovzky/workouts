...
App admins create, edit, and delete the shared content catalog: equipment, categories, exercises, and workout templates.

## Current Behavior

1. A user with the `Admin` role sees an **Admin** link in the desktop and mobile navigation. Non-admins never see it.
2. Admin opens **Admin/Index** and picks a section: Equipment, Categories, Exercises, or Workout templates.
...
6. On save the record is persisted through the REST API and the list refreshes.
7. Deleting prompts a confirmation dialog before removing the record.

## Business Rules

- All admin actions require the authenticated user to have the `Admin` role. Enforced by the `admin` route middleware and by a per-model policy (`EquipmentPolicy`, `CategoryPolicy`, `ExercisePolicy`, `WorkoutTemplatePolicy`); non-admins receive `403`.
...
- Write operations run in a database transaction; exercise category sync, template activities/sets, and translation writes commit together.

## Known Limitations

- Programs are not yet manageable here (planned as a later phase).
...
- Pages: `Admin/Index`, `Admin/EquipmentIndex`, `Admin/CategoryIndex`, `Admin/ExerciseIndex`, `Admin/WorkoutTemplateIndex`
- Web route names: `admin.index`, `admin.equipment`, `admin.categories`, `admin.exercises`, `admin.workout-templates`
- API route names: `api.v1.admin.equipment.*`, `api.v1.admin.categories.*`, `api.v1.admin.exercises.*` (`index`, `store`, `update`, `destroy`); `api.v1.admin.workout-templates.*` (`index`, `show`, `store`, `update`, `destroy`)
